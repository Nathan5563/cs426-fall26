...
func TestMergeOrCancel(t *testing.T) {
	...
	t.Run("completes with nil error when not canceled", func(t *testing.T) {
		a := make(chan string, 1)
		b := make(chan string, 1)
		out := make(chan string, 10)
		a <- "a"
		...
		err := lab0.MergeChannelsOrCancel(context.Background(), a, b, out)
		require.NoError(t, err)
		require.ElementsMatch(t, []string{"a", "b"}, chanToSlice(out))
	})

	t.Run("cancel while blocked on full out", func(t *testing.T) {
		ctx, cancel := context.WithCancel(context.Background())
		...
		require.Equal(t, []string{"a1"}, chanToSlice(out))
	})

	t.Run("deadline exceeded while inputs idle", func(t *testing.T) {
